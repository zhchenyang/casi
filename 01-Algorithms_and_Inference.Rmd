...
一种现代的基于计算机的算法 lowess 在图 1.2 中产生了有点不平坦的回归曲线。lowess 算法将注意力放在 x 轴上，将不同程度的局部多项式曲线拟合到附近 (x, y) 点上。(调用lowess中的1/3 (x,y,1/3) 决定了局部的定义。)在x轴上反复传递可以细化拟合，减少偶尔异常点的影响。图1.2中的拟合曲线在右边几乎是线性的，但在左边更为复杂，因为那里的点更加密集。它在25岁到35岁之间没有变化，这与图 1.1 所示的一致下降有潜在的重要区别。
